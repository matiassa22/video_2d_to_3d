...
def generate_stereo(in_dir, depth_dir, depth_prefix, out_dir, filename):
    ...
    cv2.imwrite(os.path.join(out_dir, "stereo" + filename + ".jpg"), result)
    # cv2.imshow("preview", result)
    # cv2.waitKey(0)


def generate_stereo_pair(filename):
    arg_parser = ArgumentParser()
    arg_parser.add_argument("-i", "--in-dir", help="Input directory for source images", default="example")
    arg_parser.add_argument("-d", "--depth-dir", help="Directory of depth maps", default="stereo")
    arg_parser.add_argument("-p", "--depth-prefix", help="Prefix of file name for depth maps", default="dp")
    arg_parser.add_argument("-o", "--out-dir", help="Output directory for stereo images", default="temp")
